 ...
 * 
 */
public class SpringJpaDAOTestGenerator implements TextFileGenerator {
    ...
    public TextFile generate(final Class aClass, final CodeGeneratorConfiguration aCodeGeneratorConfiguration)
        ...
        String idContainerClass = getIdContainerClass(persistableClass);
        String entityId = aCodeGeneratorConfiguration.getClassIdMap().getId(persistableClass.getName());

        MetaTemplate daoTestTemplate = (MetaTemplate) MetaTemplateSystem.get(TemplatesNames
                .getName("TEST_JPA_HIBERNATE_SPRING_DAO"));
        ...
        MetaTemplateGeneration metaTemplateGeneration = new MetaTemplateGeneration(daoTestTemplate, metaVariablesValue);
    ...
    private String getIdContainerClass(final Class aClass) {

        if (aClass.getSuperclass().getName().equalsIgnoreCase("java.lang.Object") || aClass.getSuperclass() == null)
